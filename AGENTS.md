...
4. **`src/commands/`** holds one file per command, each exposing `run(env, args, out, err) u8`, registered in `commands/root.zig` (the `Command` enum and the `run` switch) and in the usage text in `src/cli.zig`. `commands/migrations.zig` holds what the database commands share: loading `ffmig.toml` and the migration files, connecting, setting the `[migration]` timeouts, taking the migration lock (a PostgreSQL advisory lock that `migrate` and `rollback` hold for the whole run), reading the tracking table (upgrading one made by an older ffmig) and comparing file checksums with it, and running one migration together with its tracking row, in a transaction unless the migration has `transaction: false`. `commands/database.zig` does the same for create, drop, protect and unprotect, which connect to the maintenance database (`db.admin`).
